...
function Login() {
  ...
  const handleLogin = async () => {
    ...
    if (Object.keys(newErrors).length === 0) {
      try {
        // console.log("กำลังยิง API ไปที่หลังบ้าน...");
        const response = await axios.post("http://localhost:3000/auth/login", {
          email: formData.email,
          password: formData.password,
        });

        // console.log("ผลลัพธ์จากหลังบ้าน:", response.data);

        if (response.data.access_token) {
          // console.log(response.data.fName);

          setUser({
            name: response.data.fName || "ผู้ใช้งาน",
            isLoggedIn: true,
          ...
          // console.log("เก็บ Token สำเร็จ กำลังจะไปหน้า Home");
          alert("ยินดีต้อนรับเข้าสู่ระบบค่ะ!");
          navigate("/"); // ไปหน้า Home ต่อไป
        }
      } catch (err) {
    ...
    <div className="min-h-screen bg-white flex flex-col items-center px-6 py-20 font-sans">
      <div className="w-full max-w-[400px] space-y-8">
        <div className="text-left">
          <h1 className="text-[32px] font-black text-slate-900 italic">
            Welcome Back!
          </h1>
          <p className="text-[14px] text-[#8B8B8B]">
            ลงชื่อเข้าใช้งานเพื่อไปต่อกันเลย
          </p>
        </div>
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <div className="text-right">
              <span className="text-[11px] text-[#212587] font-bold cursor-pointer hover:underline">
                Forgot Password?
              </span>
            </div>
          </div>
        ...
        <div className="relative flex items-center py-2">
          ...
          <span className="flex-shrink mx-4 text-[12px] text-[#8B8B8B]">
            or sign in with
          </span>
          <div className="flex-grow border-t border-[#D8D8D8]"></div>
        </div>
        ...
        <div className="text-center text-[12px] pt-4">
          ...
          <span
            ...
            className="text-[#212587] font-bold cursor-pointer hover:underline"
          >
            Sign Up
          </span>
        </div>
